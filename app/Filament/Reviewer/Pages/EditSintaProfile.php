...
class EditSintaProfile extends Page implements HasForms
{
    ...
    protected static string $view = 'filament.reviewer.pages.edit-sinta-profile';
    protected static ?string $title = 'Ubah Profil Sinta';
    protected static bool $shouldRegisterNavigation = false;
    public ?array $data = [];
    public ?Reviewer $record;

    public function mount(): void
    {
        $this->record = Reviewer::find(Auth::id());

        // Jika record Reviewer belum ada...
        if (!$this->record) {
            $this->record = Reviewer::create(['id' => Auth::id()]);

            Notification::make()
                ->title('Selamat datang! Mohon lengkapi profil Anda.')
                ->info()
                ->send();
        }
